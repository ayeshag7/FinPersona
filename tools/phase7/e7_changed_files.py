"""
v2.1 Phase 7 -- PHASE_7_CHANGED_FILES.md generated from git and verified against disk (the execution prompt's rule).

    python -m tools.phase7.e7_changed_files [--base 0b2c48c] [--check]

Every path is the diff between the Phase-6 hand-over commit and HEAD plus the untracked files under the phase's
...
DOC = os.path.join(ROOT, "docs", "env_v2", "v2_1", "PHASE_7_CHANGED_FILES.md")
BASE_DEFAULT = "22d572d"     # the Phase-6 close-out commit
IGNORE = ("docs/env_v2/generated/v2_1/e6_1/cache/", "COMMIT",
          "docs/env_v2/generated/v2_1/e7_1/_scratch/",
          "docs/env_v2/v2_1/PHASE_7_CHANGED_FILES.md")      # the list itself (its own status flips on every write)
...
def render(base: str) -> str:
    ...
    groups = {"envs/": [], "evaluation/": [], "simulation/": [], "tools/": [], "tests/": [], "docs/env_v2/v2_1/": [], "docs/env_v2/spec/": [],
    ...
    L = [f"# Phase 7 — files written or changed (generated from git `{base}..{head}` and the working tree by "
         f"`tools/phase7/e7_changed_files.py`; verified against disk by `--check`)", "",
...
def main(argv=None):
    ...
    if a.check:
        ...
        if strip(cur) != strip(text):
            print("PHASE_7_CHANGED_FILES.md is stale"); return 1
        print("PHASE_7_CHANGED_FILES.md matches git and disk"); return 0
