...
| `history/` | **`SYNTHETIC_ENVIRONMENT_HISTORY.pdf` / `.docx` / `.md`**: the whole history of the environment in one document, v1 through v2 to the ten phases of v2.1, with what each version was, what was found wrong with it, what changed, the evidence behind every change and which positions are held by design rather than by a fit. 63 pages, 33 figures, every number carrying its sample size. Built by `tools/history_figures.py` (figures, from the generated result files) and `tools/history_docx.py` (document and PDF). The figure PNGs are not tracked: the blanket `figures/` rule in `.gitignore` excludes them, and the docx and PDF embed them | yes (the figure PNGs: no) |
...
For the whole story in one place, read `history/SYNTHETIC_ENVIRONMENT_HISTORY.pdf` instead of steps 1 to 5: it covers v1, v2 and every v2.1 phase, and ends with the environment as it stands. The phase reports remain the primary sources it cites.

## The grid runner branch

The finalised environment also lives on the branch `synthetic-env-grid`, cut from `main` by deleting everything the main LLM grid does not need: no phase reports, no pre-registrations, no decision log, no analysis tools, no v1 generator, no old results, no data panel. What stays is the generator and its fitted parameters, the harness, the scoring layer, the grid tooling, the tests and a README of its own. It exists so that a long paid run happens against a small, reviewable surface, with no doubt about which code produced it. It is a branch of this repository and not a fork: the environment code hash is identical to `main`'s (`8dc4c7d0c9817dcd` over the same 30 frozen files) and the 95 path configurations reproduce the frozen reference with 0 changed. Nothing is developed there that is not also here.

**The branch is behind `main` by one commit and must be brought level before the grid runs.** The pre-grid freeze recorded the team decisions of 14 Sep 2026 in `envs/v2/params/events.json` and `observables.json` and rewrote `tests/v2_freeze_manifest.json`; the hash moved from `03ce96aadef0f608` to `8dc4c7d0c9817dcd`. The change is metadata only — 0 of 95 path configurations moved — but until those three files are ported, the two branches report different `Env_Code_Hash` values in their run rows, which is the one thing this branch exists to make unambiguous.
