...
| **The box** | every number here is the laptop's; the 267 files are on the box with the committed sizes; the reproduction check (`box_bootstrap.sh env,panel,hashes,refrow`) waits for the tunnel; if it agrees, the box runs what Phase 7 needs at 24 workers | Fan (the tunnel); `box_stage2.sh 504cacf` (the phase) |
