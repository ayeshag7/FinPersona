...
invested); the t = 0 gate table is in `pilot_report_gate_common_start.csv`.
...
MCR (mandate-conditional regret, lower better; normalised 0 = best trivial policy, 1 = mandate-conditional oracle):
...
mixed-effects table is not meaningful with one model).
