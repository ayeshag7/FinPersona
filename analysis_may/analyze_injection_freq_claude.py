"""
Injection-Frequency Ablation Analysis -- claude-sonnet-4-6
FinPersona-Bench -- Mandate Refresh Cost-Benefit Pareto Curve

Reads:
  analysis_may/outputs/injection_freq/claude-sonnet-4-6/injection_freq_curve.csv
    (P(intended persona) per k x persona x scenario, n=600 rationales per cell)
  results_may/injection_freq/master_summary_claude_20260526_195801.csv
    (run-level MAS_Deviation and financial metrics per k)

Writes (to analysis_may/outputs/injection_freq/claude-sonnet-4-6/):
...
Usage (from project root):
    python analysis_may/analyze_injection_freq_claude.py
"""
...
OUT_DIR = HERE / "outputs" / "injection_freq" / "claude-sonnet-4-6"
OUT_DIR.mkdir(parents=True, exist_ok=True)

CURVE_CSV   = OUT_DIR / "injection_freq_curve.csv"
SUMMARY_CSV = PROJECT_ROOT / "results_may" / "injection_freq" / "master_summary_claude_20260526_195801.csv"

K_ORDER      = [1, 5, 25, 100, float("inf")]
K_LABELS     = {1: "k=1", 5: "k=5", 25: "k=25", 100: "k=100", float("inf"): "k=INF"}
K_INJECTIONS = {1: 200, 5: 40, 25: 8, 100: 2, float("inf"): 0}
...
def compute_pareto(curve: pd.DataFrame, run_df: pd.DataFrame) -> dict:
    ...
    for persona in PERSONAS:
        ...
        for k in K_ORDER:
            ...
            overhead_pct   = injections / 200 * 100
...
def compute_plateau(curve: pd.DataFrame) -> dict:
    ...
    for persona in PERSONAS:
        ...
        for thresh in thresholds:
            best_k = None
            for k in sorted(K_ORDER, reverse=True):
                if pct[k] >= thresh:
                    best_k = k
                    break
...
def main():
    ...
    run_df["Injection_Frequency"] = pd.to_numeric(
        run_df["Injection_Frequency"].replace("INF", float("inf")), errors="coerce"
    )
    ...
    summary = {
        ...
        "model":        MODEL,
        "paper_context": (
            f"Replicates Qwen2.5-7B injection-frequency ablation on {MODEL}. "
            "Ablates k in {1, 5, 25, 100, INF}, flat scenario, 3 personas x 3 seeds. "
    ...
    print("\n--- Pareto curve: P(intended persona) [" + MODEL + "] ---")
    ...
    for persona in PERSONAS:
        ...
        print(f"  {persona}: recommended k={r['recommended_k']}  ({r['token_cost_at_recommended_k']})")
        print(f"    {r['rationale']}")
